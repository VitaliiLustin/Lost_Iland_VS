package wins 
{
	...
	public class PortOrderWindow extends Window
	{
		...
		private function createWait():void 
		{
			...
			//separator = Window.backingShort(326, 'separator3');
			//separator.alpha = 0.5;
			//separator.x = (settings.width - separator.width) / 2;
			//separator.y = settings.height - 123;
...
internal class TradeItem extends Sprite {
	...
	public function TradeItem(mode:int, window:PortOrderWindow, id:int):void {
		...
		//if (mode != MODE_EMPTY && window.mode == PortOrderWindow.MODE_DONE)
			//drawBttn();
	}
	...
	//{
		//bttnTake = new Button( {
			...
			//fontSize:	26
		//});
		//bttnTake.x = (background.width - bttnTake.width) / 2;
		//bttnTake.y = background.height + 12;
	...
	{	
		...
		if (mode != MODE_BOUGHT && !App.user.stock.canTake(dataObj))
			return;
