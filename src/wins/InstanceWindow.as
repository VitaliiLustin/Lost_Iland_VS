	...
	public class InstanceWindow extends BuildingWindow
	{
		...
		private function showPersonages():void {
			...
			for each(var pers:Object in arrPers) {
				...
				container.x = 150;
				container.y = -125;
				persItem.x = persPositions[count].x;
				persItem.y = persPositions[count].y;
		...
		public function setHeroInto(sid:int, addNow:Boolean = false):void 
		{
			...
			if(InstanceWindow.isTutorial){
				...
				/*App.user.quests.currentTarget = btnnStartReq;
				App.user.quests.lock = false;*/
				Tutorial.watchOn(btnnStartReq, 'left', false);
			}
		}
		...
		private function drawArrow():void 
		{
			...
			/*arrow = new Bitmap(textures.arrowInst);
			arrow.alpha = 0;
			bodyContainer.addChild(arrow);
			arrow.x = 20;
			arrow.y = 140;
			
			TweenLite.to(arrow, 1, {alpha:1});*/
		}
...
//
		...
		public function drawFriendsInfo():void
		{
			...
			/*var bgIcon:Bitmap = new Bitmap(Window.textures.bankRoundBaking2);
			var icon:Bitmap = new Bitmap(UserInterface.textures.friendsIcon);
			
			icon.x = (bgIcon.width - icon.width) / 2;
			icon.y = (bgIcon.height - icon.height) / 2;
			
			countFriends.x = bgIcon.width + 6;
			countFriends.y = (bgIcon.height - countFriends.textHeight) / 2;
			
			iconCont.addChild(bgIcon);
			iconCont.addChild(icon);
			
			friendsCont.addChild(iconCont);
			iconCont.x = (underBg.width - iconCont.width) / 2;
			iconCont.y = title.textHeight + 2;*/
		...
		{
			if (App.social == 'OK') {
				WallPost.makePost(WallPost.INSTANCE_INVATE, { sid:settings.target.sid, uid:App.user.id } ); 
		...
		//private function resizePers():void
		//{
			//if(container){
				//container.x = App.self.stage.stageWidth / 2 - 280;
				//container.y = (App.self.stage.stageHeight - 310) / 2 + 100;
		...
		{ 		
			...
			//container.x = App.self.stage.stageWidth / 2 - bottomsContainer.width / 2 + 165;
			//container.y = App.self.stage.stageHeight - bottomBg3.height + 25 - 77;
			
			//if (bottomBg && bottomBg.parent) {
				//bottomBg.parent.removeChild(bottomBg);
				//bottomBg = null;
			//}
			//if(container)container.visible = false;
			//var instance:Missionhouse = settings.target;
			//if (instance.focusPositions.hasOwnProperty(instance.info.view)) {
				//App.map.focusedOn({x:instance.focusPositions[instance.info.view].x, y:instance.focusPositions[instance.info.view].y}, false, resizePers, true, 1.1, false, 0.1);
			//}else {
				//App.map.focusedOn({x:instance.x + 20, y:instance.y + 50}, false, resizePers, true, 1.1, false, 0.1);
			//}
			
			
			//bottomBg = Window.backingShort2(App.self.stage.stageWidth, 310, 'missionBacking', true);
			//addChildAt(bottomBg, 0);
			//bottomBg.y = App.self.stage.stageHeight - bottomBg.height;
			...
			//friendsCont.x = App.self.stage.stageWidth/2 + 140;
			//friendsCont.y = bottomBg.y + 32;
			
			//otherCont.x = App.self.stage.stageWidth / 2;
			//otherCont.y = bottomBg.y - otherCont.height / 2 + 10;
			//
			//heroesCont.x = App.self.stage.stageWidth / 2 + 150;
			//heroesCont.y = App.self.stage.stageHeight / 2 - 200;
			//
			//improvementCont.x = App.self.stage.stageWidth / 2;
			//improvementCont.y = bottomBg.y + 90;
			//
			//exit.x = App.self.stage.stageWidth / 2 + 350;
			//exit.y = App.self.stage.stageHeight - 360;
		}
