package buttons 
{
	...
	public class Button extends LayerX
	{
		...
		protected function drawBottomLayer():void
		{
			...
			if (settings.hasDotes) {
				if (settings.greenDotes) {
					var dot1:Bitmap = new Bitmap(Window.textures.dotGreen);
					var dot2:Bitmap = new Bitmap(Window.textures.dotGreen);
				}else if (settings.grayDotes) {
					dot1 = new Bitmap(Window.textures.dotGray);
					dot2 = new Bitmap(Window.textures.dotGray);
				}else {
					dot1 = new Bitmap(Window.textures.dotYellow);
					dot2 = new Bitmap(Window.textures.dotYellow);
				}
		...
		{
			trace('onBeforeClick');
			if (Quests._lockFuckAll) {
				trace('stopImmediatePropagation1')
				e.stopImmediatePropagation();
				return;
			}
			if (Quests.lockButtons) {
				trace('stopImmediatePropagation2')
				e.stopImmediatePropagation();
