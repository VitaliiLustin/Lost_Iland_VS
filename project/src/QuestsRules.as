package  
{
	...
	public class QuestsRules 
	{
		...
		public static function glowTree():void {
			...
			App.user.quests.findTarget([92], false, {'busy':0, 'id':210});
		}
		...
		public static function glowPineTree():void {
			...
			App.user.quests.findTarget([66], false, {'busy':0, 'id':45});
		}
			...
			if (App.user.quests.currentTarget is Sphere){
				if (Window.isOpen) {
					for (i = 0; i < App.self.windowContainer.numChildren; i++) {
						...
						}else if (win is CreateAnimalWindow) {
							for each(item in win.items) {
								bttn = item.selectBttn;
								bttn.showGlowing();
								bttn.showPointing("top", (bttn.width - 30) / 2, 0, bttn.parent);
		...
		/*	if (App.user.quests.currentTarget != null) {
			...
			place = new LayerX();
			place.graphics.beginFill(0x000000, 0.4);
			place.graphics.drawEllipse(0, 0, 80, 46);
			place.graphics.endFill();
			place.filters = [new BlurFilter(15, 15, 1)];
			
			App.map.mLand.addChild(place);
			place.x = node.tile.x;
			place.y = node.tile.y;
			
			place.showPointing("top");
			*/
