package 
{
	...
	public class Zone extends LayerX
	{
		...
		public static var zonePoint:Object = { 
			180:{x:3400, y:2100},
			181:{x:3600, y:600},
			182:{x:4800, y:1200},
			183:{x:4800, y:2300},
		...
		private function drawInformer():void
		{
			...
			//var bg:Bitmap = Window.backing2(220, 100, 45, "questsSmallBackingTopPiece", "questsSmallBackingBottomPiece");
			...
			if (App.data.storage[sID]) {
				var titleZone:String = App.data.storage[sID].title;
			}else {
				titleZone = 'Noname'
			}
			var titleText:TextField = Window.drawText(titleZone, textSettings);
				titleText.width = 120;
				...
				//informer.addChild(titleTextBack);
				informer.addChild(titleText);
				
			/*var infoText:TextField = Window.drawText(Locale.__e("flash:1382952380336"), {
				fontSize:19,
				color:0x5d5d5d,
				border:false,
				textAlign:'center',
			...
			infoText.x = (bg.width - infoText.width)/2;
			infoText.y = bg.height - 88;
			
			informer.addChild(infoText);	*/
