package wins 
{
	...
	public class SaleDecorWindow extends Window 
	{
		...
		public function SaleDecorWindow(settings:Object=null) 
		{
			...
			//settings['title'] = Locale.__e('flash:1393583201665');
		...
		{
			var timeBg:Bitmap = backingShort(165, 'salseTimeBg');
			bodyContainer.addChild(timeBg);
			timeBg.x = ribbon.x + (ribbon.width - timeBg.width) / 2;
			timeBg.y = ribbon.y + (ribbon.height - timeBg.height) / 2 - 15;
			
			var timeTitle:TextField = Window.drawText(Locale.__e('flash:1393581955601'),{
				fontSize:28,
				color:0xffffff,
				borderSize:4,
				borderColor:0x5a2910
			});
			bodyContainer.addChild(timeTitle);
			timeTitle.x = timeBg.x + (timeBg.width - timeTitle.width) / 2;
			timeTitle.y = timeBg.y - 15;
			
			var time:int = currentSale.duration * 60 * 60 - (App.time - currentSale.time);
			...
			timerText.width = 230;
			timerText.x = timeBg.x + (timeBg.width - timerText.width) / 2;
			timerText.y = timeBg.y + (timeBg.height - timerText.height) / 2 + 15;
		...
		override public function contentChange():void {
			...
			for (var i:int = paginator.startCount; i < paginator.finishCount; i++){
			{
				...
				container.y = (settings.height - container.height) / 2 - 5;
			}
...
internal class DecorItem extends Sprite {
		...
		private function drawText():void
		{
			...
			var description:TextField = Window.drawText(App.data.storage[sID].description,{
				fontSize	:22,
				color		:0x734326,
				borderSize	:4,
		...
		private function drawSpecialChest():void
		{
			...
			if (decorItem.type == 'Golden') {
				...
				btm.smoothing = true;
				btm.x = bg.x + bg.width - 75;
				btm.y = bg.y + bg.height - 60;
			}
		}
