package wins 
{
	...
	public class MoneyHouseWindow extends Window
	{
		...
		private function drawBttn():void 
		{
			...
			if (settings.info.sID == 177) 
			{
				upgradeBttn.y = settings.height + 85;
			}	else
		...
		{
			new AskWindow(AskWindow.MODE_PUT_IN_ROOM,  { 
				title:Locale.__e('flash:1393580021031'), 
				inviteTxt:Locale.__e("flash:1382952379977"), 
				desc:Locale.__e("flash:1393581355710"),
				descY:5,
				height:530,
				target:settings.target
			},  putFriendInto ).show();
...
internal class FriendItem extends Sprite {
	...
	private function checkOnLoad():void 
	{
		...
		if (App.user.friends.data[friendId].hasOwnProperty('first_name'))
		{
	...
	private function drawAvatar():void
	{
		...
		Log.alert(App.user.friends.data);
		new AvaLoad(App.user.friends.data[friendId].photo, onAvaLoad);
	}
