package wins 
{
	...
	public class ProductionWindow extends BuildingWindow
	{
		...
		private function findTarget():void {
			...
			for (var i:int = 0; i < _arrCraft.length; i++ ) {
				if (/*settings.crafting[i]*/App.data.crafting[_arrCraft[i].fid].out == settings.find) {
					if (i >= settings.itemsOnPage) {
						var count:int = Math.ceil((i-5) / /*settings.itemsOnPage*/2);
		...
		public function initContent(settings:Object):void {
			...
			if (settings.target.info.hasOwnProperty('devel') ) {
				for (var _id:String in settings.target.info.devel.craft) {
					var obj:Object = settings.target.info.devel.craft[_id];
					lvlRec = int(_id) - (settings.target.totalLevels - settings.target.craftLevels);
					
					var counter:int = 0;
					for each(var fID:* in obj) {
						
						if (updtItems.indexOf(App.data.crafting[fID].out) != -1) {
							continue;
						}
		...
		{  
			...
			productionQueue.x = (settings.width - productionQueue.width/2) / 2 - 130;
		...
		private function addScrollItems(type:int):int
		{
			...
			for (var i:int = 0; i < countScrollItems; i++ ) {
				...
				if (settings.find == _arrCraft[newInd].fid)
					isHelp = true;
				item.change(_arrCraft[newInd].fid, _arrCraft[newInd].lvl);
				item.visible = true;
