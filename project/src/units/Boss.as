package units 
{
	...
	public class Boss extends AUnit
	{
		...
		private function showIcon(typeItem:String, callBack:Function, mode:int, btmDataName:String = 'productBacking2', scaleIcon:Number = 0.6):void 
		{
			...
			//var test:int = App.data.storage[App.data.storage[sid]["in"]];
			//App.user.stock
			cloudAnimal = new AnimalCloud(callBack, this, sid/*App.data.storage[require]*/, mode/*, {scaleIcon:scaleIcon}*/);
		...
		private static function addBoss():void
		{
			...
			var lottery:int = Math.round(Math.random() * 1);
			...
			new CharactersWindow( { quest: { character:6, title:Locale.__e('flash:1409299679964'), description:txt }} ).show();;
		...
		public override function click():Boolean {
			...
			if (clickCounter == 1) {
				...
				showIcon('require', click, AnimalCloud.MODE_NEED, 'productBacking2'/*, 0.7*/);
				clickCounter = 1;
				return true;
			}
